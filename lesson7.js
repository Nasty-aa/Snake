...
function startGame() {
    ...
    setTimeout(createBlocked, 5000);
    var block = document.querySelector('#snake-start');
    block.removeEventListener('click',startGame);
    block.className = " lockeds";
    var blockTotal = document.getElementsByClassName('h1')[0];
    blockTotal.innerText = "Ваш счёт: "+score;
}
...
function move() {
   ...
   if (new_unit == undefined) {
     ...
     if(!isSnakeUnit(new_unit) && !new_unit.classList.contains('blocked-unit')){
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        var blocked1 = document.getElementsByClassName('blocked-unit')[0];
        blocked1.classList.remove('blocked-unit');
        createFood();
        createBlocked();
        score++;
        var blockTotal = document.getElementsByClassName('h1')[0];
...
function createBlocked() {
    ...
    while (!blockedCreated) { //пока еду не создали
        // рандом
        var food_x = Math.floor(Math.random() * FIELD_SIZE_X);
        var food_y = Math.floor(Math.random() * FIELD_SIZE_Y);
        ...
        if (!food_cell_classes.includes('snake-unit') && !food_cell_classes.includes('food-unit')) {
            ...
            food_cell.setAttribute('class', classes + 'blocked-unit');
            blockedCreated = true;
        }
    }
}
